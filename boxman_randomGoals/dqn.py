...
class ComposedDQN(nn.Module):
    ...
    def MIN(self, dqn,s,g):
        rmin = -0.1
        dqn_min = [0]*self.n_action
        
        sg = torch.cat((s,g),dim=3)
        dqn_sg = dqn(sg).cpu()
        gg = torch.cat((g,g),dim=3)
        dqn_gg = dqn(gg).cpu()
        
        c = rmin-dqn_gg.min()
        for a in range(self.n_action):
            if s.sum() != g.sum():
                dqn_min[a] = dqn_sg[a] + c
            else:
                dqn_min[a] = rmin       
        return dqn_min
    
    def MAX(self, dqn,s,g):
        rmax = 2
        dqn_max = [0]*self.n_action
        ...
        for a in range(self.n_action):
            if s.sum() != g.sum():
                dqn_max[a] = dqn_sg[a] + c
            else:
                dqn_max[a] = rmax        
        return dqn_max
    
    def NOT(self, dqn,s,g):
        dqn_max = self.MAX(dqn,s,g)
        dqn_min = self.MIN(dqn,s,g)
        
        sg = torch.cat((s,g),dim=3)
        dqn_sg = dqn(sg).cpu()
        
        dqn_not = [0]*self.n_action
        for a in range(self.n_action):
           dqn_not[a] = (dqn_max[a]+dqn_min[a]) - dqn_sg[a]    
        return dqn_not
    
    def OR(self, dqn1, dqn2, s, g):
        dqn2_not = self.NOT(dqn2,s,g)
        
        sg = torch.cat((s,g),dim=3)
        dqn1_sg = dqn1(sg).cpu()
        dqn2_sg = dqn2(sg).cpu()
        
        dqn_or = [0]*self.n_action
        for a in range(self.n_action):
            dqn_or[a] = max(dqn1_sg[a],dqn2_sg[a])
        return dqn_or
    
    def AND(self, dqn1, dqn2, s, g):
        dqn2_not = self.NOT(dqn2,s,g)
        ...
        dqn_and = [0]*self.n_action
        for a in range(self.n_action):
            dqn_and[a] = min(dqn1_sg[a],dqn2_sg[a])
        return dqn_and

    def __init__(self, dqns, weights=None, compose="or"):
        super(ComposedDQN, self).__init__()
        self.n_action = dqns[0].n_action
        self.dqns = dqns
        if weights is None:
    ...
    def forward(self, x):
        ...
        if self.compose=="not":
            q = self.NOT(self.dqns[0], s, g)
        else:    
            dqn1, dqn2 = self.dqns
            if self.compose=="and":
                q = self.AND(dqn1, dqn2, s, g)
            else: #self.compose=="or":
                q = self.OR(dqn1, dqn2, s, g)
