...
if __name__ == '__main__':
    ...
    P = lambda x: x.colour=="purple"
    B = lambda x: x.colour=="blue"
    S = lambda x: x.shape=="square"
    env = MaxLength(WarpFrame(CollectEnv(start_positions =None, goal_condition=lambda x: (B(x) or S(x)) and not (B(x) and S(x)))),
                     max_trajectory)
    ...
    dqn_or = ComposedDQN([dqn_purple,dqn_blue], compose="or")
    dqn_or_bc = ComposedDQN([dqn_blue,dqn_crate], compose="or")
    dqn_and = ComposedDQN([dqn_blue,dqn_crate], compose="and")
    dqn_not_and = ComposedDQN([dqn_and], compose="not")
    dqn_xor = ComposedDQN([dqn_or_bc,dqn_not_and], compose="and")
